  ...
  const struct fdt_cb *cb)
{
  ...
  while (1) {
    switch (bswap(lex[0])) {
      case FDT_NOP: {
        //printm("Enter EDT_NOP\n");
        lex += 1;
        break;
      }
      case FDT_PROP: {
        //printm("Enter FDT_PROP\n");
        assert (!last);
      ...
      case FDT_END_NODE: {
        //printm("Enter FDT_END_NODE\n");
        if (!last && node && cb->done) cb->done(node, cb->extra);
        return lex + 1;
      }
      default: { // FDT_END
        //printm("Enter default\n");
        if (!last && node && cb->done) cb->done(node, cb->extra);
        return lex;
      }
    }
...
void query_mem(uintptr_t fdt)
{
  ...
  mem_size = 0;
    //printm("Enter query_mem\n");
  fdt_scan(fdt, &cb);
   // printm("before assert mem_size %ld\n", mem_size);
  assert (mem_size > 0);
}
...
//    printm("Enter hart_open, scan->cpu = %d\n", scan->hart);
    if (!scan->cpu) 
    {
        scan->hart = -1;
    }
...
{
    struct hart_scan *scan = (struct hart_scan *)extra;
//    printm("Enter hart_done\n");
//    printm("scan->cpu = %d, node = %p, scan->hart = %d\n", scan->cpu, node, scan->hart);
//    printm("scan->controller = %p, scan->cells = %d, scan->phandle \n", scan->controller, scan->cells, scan->phandle);
    if (scan->cpu == node) {
//        printm("enter if 1\n");
        assert (scan->hart >= 0);
    }
